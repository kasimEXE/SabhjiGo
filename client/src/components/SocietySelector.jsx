...
import { updateUserSociety, getUserData } from "../data/users";
...
function SocietySelector() {
  ...
  // Load current society from Firestore
  useEffect(() => {
    const loadUserSociety = async () => {
      if (!user) {
        setLoading(false);
        return;
      }
      ...
      try {
        ...
        if (userData?.societyId) {
          setSelectedSociety(userData.societyId);
        } else {
          // Set default if no society selected
          setSelectedSociety('sunshine-apartments');
        }
      } catch (error) {
        console.error('Error loading user society:', error);
        // Fallback to default
        setSelectedSociety('sunshine-apartments');
      } finally {
        setLoading(false);
